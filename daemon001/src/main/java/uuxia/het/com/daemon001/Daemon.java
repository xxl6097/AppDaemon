...
public class Daemon {
    ...
    private static final String DAEMON_BIN_NAME = "daemon.lock";
    ...
    private static void start(Context context, Class<?> daemonClazzName) {
        ...
        Log.e("uulog.jnijj",cmdBuilder.toString());
        try {
    ...
    public static void run(final Context context, final Class<?> daemonServiceClazz) {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                start(context, daemonServiceClazz);
            }
        }).start();
    }

    public static void run(final Context context, final String daemonServiceClazz) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                Command.install(context, BIN_DIR_NAME, DAEMON_BIN_NAME);
